    ...
    public resolveID = async (resolvable: string, type: string) => {
        ...
        if (resolvable.includes("youtube.com") || resolvable.includes("youtu.be")) {
            if (resolvable.includes("channel")) {
                id = String(resolvable.match(/(channel\/)(.*?)(?=(?:\?|\/|$))/)[2])
            } else if (resolvable.includes("c/")) {
                const username = String(resolvable.match(/(c\/)(.*?)(?=(?:\?|\/|$))/)[2])
                ...
                const username = String(resolvable.match(/(user\/)(.*?)(?=(?:\?|\/|$))/)[2])
                ...
                id = String(resolvable.match(/(watch\?v=)(.*?)(?=(?:&|$))/)[2])
            } else if (resolvable.includes("youtu.be")) {
                id = String(resolvable.match(/(youtu.be\/)(.*?)(?=(?:\?|$))/)[2])
            } else if (resolvable.includes("playlist?list=")) {
                id = String(resolvable.match(/(playlist\?list=)(.*?)(?=(?:\?|$))/)[2])
            } else {
                const query = String(resolvable.match(/(youtube.com\/)(.*?)(?=(?:\?|$))/)[2])
